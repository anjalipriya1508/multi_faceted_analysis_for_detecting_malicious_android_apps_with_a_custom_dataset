...
dataset = pd.read_csv('../androZoo_dataset_analysis/CICDataset_MalDroid_2020.csv').iloc[:, 1:]
...
def print_metrics(name, y_true, y_pred, y_prob, multiclass=True):
...
# ---------- Manual Decision Tree and Random Forest ----------
class DecisionTree:
...
class RandomForest:
    ...
    def predict_proba(self, X):
        ...
        for i, cls in enumerate(self.classes):
            proba[:, i] = np.mean(all_preds == cls, axis=0)
        return proba
...
builtin_probs = builtin_rf.predict_proba(X_test)
...
summary_df[['Accuracy', 'Precision', 'Recall', 'F1-Score', 'AUC Score']] = summary_df[
