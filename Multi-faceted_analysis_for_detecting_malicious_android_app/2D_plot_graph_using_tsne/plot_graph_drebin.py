...
# === Load CSV ===
csv_path = "../androZoo_dataset_analysis/drebin.csv"  # <-- Replace with your actual file
...
# === Define label colors ===
label_colors = {
    'b': 'darkgreen',  # Place B (goodware)
    's': 'darkred'     # Place S (malware)
}
...
for label in df_tsne['class'].unique():
    subset = df_tsne[df_tsne['class'] == label]
    color = label_colors.get(label.lower(), 'gray')
    plt.scatter(subset['x'], subset['y'], label=str(label).upper(), c=color, alpha=0.7, s=20)

plt.title("t-SNE Visualization (Place B vs Place S)")
...
# === Save plot ===
plot_path = os.path.join(output_folder, "drebin_dataset_plot.png")
